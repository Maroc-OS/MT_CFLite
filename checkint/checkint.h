...
#define __CHECKINT_CXX_OVERFLOW_FUNC(return_type, t1, t2, oper, signedness)           \
  __CHECKINT_INLINE return_type##_t __checkint_##return_type##_##oper(t1##_t x, t2##_t y, int32_t* err) { \
    return __checkint_ ## return_type ## _ ## oper ## _ ## signedness(x, y, err); \
  }

#define __CHECKINT_CXX_OVERFLOW_FUNCS(type, oper) \
  __CHECKINT_CXX_OVERFLOW_FUNC(type, type, type, oper, signed_signed) \
  __CHECKINT_CXX_OVERFLOW_FUNC(type, type, u##type, oper, signed_unsigned) \
  __CHECKINT_CXX_OVERFLOW_FUNC(type, u##type, type, oper, unsigned_signed) \
  __CHECKINT_CXX_OVERFLOW_FUNC(type, u##type, u##type, oper, unsigned_unsigned) \
  __CHECKINT_CXX_OVERFLOW_FUNC(u##type, type, type, oper, signed_signed) \
  __CHECKINT_CXX_OVERFLOW_FUNC(u##type, type, u##type, oper, signed_unsigned) \
  __CHECKINT_CXX_OVERFLOW_FUNC(u##type, u##type, type, oper, unsigned_signed) \
  __CHECKINT_CXX_OVERFLOW_FUNC(u##type, u##type, u##type, oper, unsigned_unsigned)


__CHECKINT_CXX_OVERFLOW_FUNCS(int64, add)
__CHECKINT_CXX_OVERFLOW_FUNCS(int64, sub)
__CHECKINT_CXX_OVERFLOW_FUNCS(int64, mul)
__CHECKINT_CXX_OVERFLOW_FUNCS(int32, div)
__CHECKINT_CXX_OVERFLOW_FUNCS(int64, div)

#undef __CHECKINT_CXX_OVERFLOW_FUNCS
#undef __CHECKINT_CXX_OVERFLOW_FUNC
