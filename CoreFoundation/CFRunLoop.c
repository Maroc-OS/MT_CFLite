...
CF_EXPORT pthread_t _CFMainPThread(void);
#define pthread_main_thread_np() _CFMainPThread()

// dispatch_get_main_queue_eventfd_np renamed to dispatch_get_main_queue_handle_np :)
extern dispatch_queue_handle_t dispatch_get_main_queue_handle_np(void);
extern void dispatch_main_queue_drain_np(void);

CF_EXPORT dispatch_queue_handle_t dispatch_get_main_queue_handle_np(void);
CF_EXPORT void dispatch_main_queue_drain_np(void);

DISPATCH_EXPORT dispatch_queue_handle_t _dispatch_get_main_queue_port_4CF(void);
DISPATCH_EXPORT void _dispatch_main_queue_callback_4CF(void);

typedef dispatch_queue_handle_t __CFPort;
...
#elif DEPLOYMENT_TARGET_WINDOWS || DEPLOYMENT_TARGET_IPHONESIMULATOR
...
#if DEPLOYMENT_TARGET_LINUX
	dispatchPort = dispatch_get_main_queue_handle_np();
#else
	dispatchPort = _dispatch_get_main_queue_port_4CF();
#endif
	}
#if USE_DISPATCH_SOURCE_FOR_TIMERS
    __CFPort modeQueuePort = CFPORT_NULL;
    if (rlm->_queue) {
...
#if USE_DISPATCH_SOURCE_FOR_TIMERS
        else if (modeQueuePort != CFPORT_NULL && livePort == modeQueuePort) {
